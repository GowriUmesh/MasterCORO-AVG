...
class VisualOdometry:
    ...
    def compute_relative_transform(self, matches: List[cv2.DMatch], update_src_frame: bool) -> \
        ...
        #0, [np.array([])], [np.array([])], [np.array([])]  # TODO
        if self.print_info:
        ...
        if rots:
            ...
            if self.plane_d_src < 0:
                ...
                self.plane_d_src = np.dot(normals[0].T, self.plane_origin_src)  # TODO: compute distance from the 1st camera frame C0 to the plane
            ...
            trans[0] *= self.plane_d_src  # TODO: scale the translation vector with self.plane_d_src
            ...
            # TODO: (continue) incoming csamera frame using trans[0] and rots[0]
            incoming_M_src[:3, : 3] = rots[0]
            incoming_M_src[:3, 3] = trans[0].flatten()
            print(incoming_M_src)
            if update_src_frame:
                ...
                self.plane_normal_src = incoming_M_src[:3, 2].reshape(-1, 1)  # TODO
                ...
                self.plane_d_src = np.dot(normals[0].T, incoming_M_src[:3, -1].reshape(-1, 1)) # TODO
